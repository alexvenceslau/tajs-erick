...
describe("E2E Test Suite", () => {
  describe("E2E tests for server in an non-test env", () => {
  ...
  describe("E2E Tests for Server", () => {
    ...
    beforeAll(async () => {
      ...
      _testServerAdress = `http://localhost:${serverInfo.port}`;
    });

    afterAll((done) => _testServer.close(done));

    it("should return 404 from unsuported routes", async () => {
      const response = await fetch(`${_testServerAdress}/unsuported`, {
        method: "POST",
      });
    ...
    it("should return 400 and missing field message when body is invalid", async () => {
      ...
      const response = await fetch(`${_testServerAdress}/persons`, {
        method: "POST",
